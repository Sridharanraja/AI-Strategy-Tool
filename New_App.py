...
def step3():
    ...
    if st.button("Execute"):
        with st.spinner("Thinking..."):
            ...
            full_prompt = f"""
            Context: {context}
            For the Identified use cases in the previous step, Categorize the AI use cases  into 4 buckets based on the principles of the effort-impact matrix: {st.session_state.step2}
            """
            with st.spinner("Generating AI response..."):
...
def step4():
    ...
    if st.button("Execute"):
        with st.spinner("Thinking..."):
            ...
            full_prompt = f"""
            Context: {context}
            For the use cases in the buckets high effort, high impact and low effort, high impact, develop a detailed AI strategy and implementation the action plan. don't do this for Low impact, High effort and Low impact, Low effort use cases {st.session_state.step3}
            """
            with st.spinner("Generating AI response..."):
